...
被连接的设备称为服务器，连接的设备称为本地

需要在服务器防火墙中打开的端口：`22`。通过SSH连接到远程Linux服务器：

```bash
# 本地输入：用户名@地址
# 地址可以是IP地址或者域名
ssh username@192.168.1.100            # 连接至192.168.1.100服务器

# 本地输入指定端口连接
ssh -p 2222 username@example.com      # 使用非默认端口（默认22）

# 本地输入：文件传输
scp local.txt username@192.168.1.100:~/            # 上传文件到远程
scp username@192.168.1.100:~/remote.txt ./         # 下载远程文件
scp -r folder/ username@192.168.1.100:~/           # 传输整个文件夹
```
...
# 本地连接上服务器后输入：将公钥加入登录白名单
cat ~/.ssh/deploy_key.pub >> ~/.ssh/authorized_keys

# 本地连接上服务器后输入：设置正确权限（过低没有权限，过高 ssh 会认为文件不安全）
chmod 700 ~/.ssh
...
# 本地输入：把服务器私钥复制到本地（替换为你的服务器信息）
scp username@192.168.1.100:~/.ssh/deploy_key ~/.ssh/deploy_key

# 本地输入：修正本地私钥权限
chmod 600 ~/.ssh/deploy_key

# 本地输入：测试 deploy_key 是否可用（应直接登录，不再需要密码）
# 以后每次都只需输入下面的命令即可
