...
[Kubernetes](https://kubernetes.io/zh-cn/docs/concepts/architecture/nodes/) 是一个开源的容器编排引擎，用来对容器化应用进行自动化部署、 扩缩和管理。在容器编排市场中占据了主导地位。

Docker 负责制造一辆辆轻便的汽车，而 Kubernetes 则是一个复杂的交通管理系统，确保这些汽车能高效、有序地运行在道路上。对容器编排感兴趣的可以前往官网深入学习。

推荐与[深入高可用系统原理与设计](https://www.thebyte.com.cn/)这本书一同阅读。
