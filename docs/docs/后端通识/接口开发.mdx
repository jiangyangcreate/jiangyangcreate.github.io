...
| 2. 请求拦截  | Request          | Middleware       | 解析与预检：解析 HTTP 报文，处理 CORS、GZip 解压、Session 状态还原。 |
| 3. 路由匹配  | Request          | Router           | 定位：根据 URL 和 Method 找到匹配的路径函数。匹配失败则直接返回 404。    |
| 4. 依赖注入  | Request          | Depends          | 前置准备：执行依赖项（如 Auth 校验、DB Session 开启）。若失败则抛出异常。  |
| 5. 参数校验  | Request          | Pydantic         | 入参验证：将数据转为模型对象并校验格式。校验失败则返回 422。               |
| 6. 业务执行  | Processing       | Path Function    | 逻辑运算：运行你编写的业务代码。若产生错误，将在此处抛出异常。                |
| 7. 异常处理  | Processing       | Exc Handler      | 异常补救：捕获业务异常，将其转换为统一格式的 JSONResponse。           |
| 8. 数据转换  | Response         | jsonable_encoder | 序列化：将 Python 对象、ORM 模型转换为 JSON 兼容的基础数据格式。      |
| 9. 响应校验  | Response         | response_model   | 出参验证：根据 Schema 强制过滤字段（如剔除密码），确保输出安全。           |
| 10. 响应拦截 | Response         | Middleware       | 后处理：最后一次修改 Header（如加签名）、计算耗时、记录访问日志。           |
| 11. 物理发送 | Response         | ASGI Server      | 传输：通过网络套接字（Socket）将字节流正式传回客户端。                 |
| 12. 后台任务 | Post-Response    | BackgroundTasks  | 扫尾：在连接断开后，异步执行耗时操作（如发送邮件、统计数据）。                |
| 13. 应用关闭 | Lifespan (End)   | lifespan         | 释放：关闭数据库连接池，清理内存资源，确保程序优雅退出。                   |
...
- GET：查 100 次，数据不会变。
- PUT：把 A 改成 B，改 100 次，结果依然是 B。
- DELETE：删掉 ID 为 1 的用户，删 1 次和 100 次，结果都是"该用户不存在"。

**非幂等 (Non-Idempotent)：**

- POST：每点一次"提交"，后端就创建一个新订单。点 100 次，产生 100 个订单。
- PATCH：不是必须幂等的（取决于实现，建议设计为幂等）。
:::
...
- GET、HEAD、OPTIONS 被称为安全方法，因为它们理论上只读，不应该引起服务器状态变化。

**不安全方法：**

- POST、PUT、PATCH、DELETE 是不安全的，因为它们会改变数据库。
:::
...
| SMTP | 发送邮件 | 邮递员把信投进邮筒 |
| DNS | 网址解析 | 查电话本寻找联系人 |
| MQTT | 物联网通信 | 智能家居传感器的小纸条 |

#### 域名/主机

当在本地开发时，域名/主机通常为`localhost`或`127.0.0.1`。
当在服务器上部署时，域名/主机通常为服务器IP地址。

#### 端口

当协议为HTTP时，端口默认为80，当协议为HTTPS时，端口默认为443。
...
"""
{
  ...
  "cookie": "csrftoken=9VZsSS9jnRLIopC5TbdIXRl3yMKhbq5c; sessionid=h8ofhxjbfysrbzbbmcnlo601zav5qxhl"
}
"""
```
...
async def catch_all(request: Request):
    ...
    cookie作为常用的请求头字段，有一个专门的属性cookies，方便你获取解析后的cookie字典。

    "cookies1": "csrftoken=9VZsSS9jnRLIopC5TbdIXRl3yMKhbq5c; sessionid=h8ofhxjbfysrbzbbmcnlo601zav5qxhl",
    "cookies2": {
...
"""
{
  ...
  "cookies1": "session_cookie=eyJ2aWV3cyI6IDJ9.aWPV2A.Aeii6ixVKsV_dtCzXUW54GTFwTs",
  "cookies2": {
    "session_cookie": "eyJ2aWV3cyI6IDJ9.aWPV2A.Aeii6ixVKsV_dtCzXUW54GTFwTs"
  }
}
"""
...
当然，实际资源与请求方式一致总是好的，但是有时候用户登录之后，你希望获取它的用户信息，包含：用户名、头像。

一个是文本、一个是小图片，此时我们一般会选择把图片编码为base64字符串，然后和用户名一起返回给前端。这样可以减少一次请求。
