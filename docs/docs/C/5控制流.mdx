...
在C中，大部分语句都以分号结尾 或 被大括号`{}`包裹。除此之外，还有用于`goto`跳转的标号语句，格式为`标号:`,例如`aa:`,`bb:`。
...
一条语句（或者至少是一条有用的语句）相当于一条完整的指令。

因此一些意想不到的结果可能是错误的添加分号（空语句）导致的。

下面代码会运行10次空语句`;`，然后运行1次复合语句`{ printf("Hello, World!\n"); }`，所以只打印1次"Hello, World!"。
