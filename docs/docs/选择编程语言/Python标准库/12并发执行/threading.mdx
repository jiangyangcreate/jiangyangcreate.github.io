...
[threading](https://docs.python.org/zh-cn/3.14/library/threading.html?highlight=threading#module-threading):Threading Multiprocessing Module Code Documentation
...
t = threading.Timer(interval= 3 ,function=lambda: print("定时器"))
t.start()
for i in range(5):
    time.sleep(1)
    if i == 0: 
