...
`__iter__()` 使对象可以被迭代（用于 for 循环）。

```python showLineNumbers
class ClothesCollection(object):
    def __init__(self):
        self.clothes = ["shirt", "pants", "jacket"]
        
    def __iter__(self):
        return iter(self.clothes)

collection = ClothesCollection()
for item in collection:
    print(item)  # 依次输出: shirt, pants, jacket
```

### \_\_add\_\_和\_\_sub\_\_
...
### \_\_rmul\_\_和\_\_mul\_\_

`__mul__()` 和 `__rmul__()` 定义对象的乘法运算行为。

`__mul__()` 定义对象的左乘法运算行为。

`__rmul__()` 定义对象的右乘法运算行为。
