...
实例方法按照是否可访问实例和类的属性分为：
- 实例方法：第一个参数传入实例（约定为self），可以访问实例的属性和方法
- 类方法：第一个参数传入类（约定为cls）+ @classmethod 装饰器，可以访问类的属性和方法
- 静态方法：@staticmethod 装饰器，不能访问实例和类的属性和方法

类方法按照是否以`__`开头和结尾，可以分为以下几种：
...
- 名称修饰方法，即以 `__` 开头不以 `__` 结尾的属性是更加特殊的方法。
...
`super()` 函数用于在子类中调用父类（或超类）的方法，特别是支持多继承的场景下，它会根据方法的解析顺序[Method Resolution Order, MRO](#__mro__与mro)来决定调用哪个类的方法。

:::info
这种将上下文打包传递给下一个可调用对象的重定向思想，在Python中非常常见，比如各类装饰器、回调函数等。
:::

#### 无参数形式

当在类的方法内部使用 `super()` 时（通常是实例方法，如 `__init__`），它会自动推断当前类和当前实例（即 `self`）。这相当于 `super(CurrentClass, self)`，其中 `CurrentClass` 是当前方法的所在类。

必须在方法内部使用（有 `self` 或 `cls` 上下文），否则会报错。无参数形式依赖于 Python 的 `__class__` 魔法变量。它会返回一个代理对象，用于调用当前类在 MRO 中的下一个类的方法（通常是直接父类，但多继承时可能不同）。

将当前的上下文(`self` 或 `cls`)作为参数，传递给"MRO的下一个类 的 指定方法",然后调用。
...
`super(class, instance)` 明确指定要查找的类和绑定的类或实例，提供最大的控制灵活性，一般是与实例方法和类方法配合使用。
