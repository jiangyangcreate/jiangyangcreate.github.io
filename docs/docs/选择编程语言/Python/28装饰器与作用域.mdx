...
`callable`函数用于检查一个对象是否是可调用的。即是否可以通过`()`调用。返回 True 或 False，用于条件判断。
...
@moretime(times = 3) # 这是第一次出现()调用的位置，直接执行，注册times参数为5，并返回 decorator 函数作为真正的装饰器
def test2():
    print('test2')
print('准备执行test2函数')
# 这是第二次出现()调用的位置，执行 decorator 函数，返回 wrapper 函数并执行
test2()
"""
{'times': 3}
准备执行test2函数
