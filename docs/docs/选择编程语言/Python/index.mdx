...
考虑到<HoverText text="AI编程的发展趋势" explanation="对绝大多数技术都能达到初级至中级工程师的水平"/>。当下的专业编程从业者应该对常用技术更加深入底层达到中高级水准，同时眼界开阔，技术广度超越模型上下文以实现跨领域举一反三。传统对函数、内存等概念浅尝辄止式的学习已无法应对当下的面试考核与业务需求。
...
- Cursor推荐更改拓展栏位置：`View` -> `Appearance` -> `Panel Position` -> `Bottom`
