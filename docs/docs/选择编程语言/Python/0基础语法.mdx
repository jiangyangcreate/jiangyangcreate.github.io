...
- stringprep: 互联网字符串预处理
- readline: GNU readline接口
- rlcompleter: GNU readline补全函数
...
- tomllib: 解析TOML文件
- netrc: netrc文件处理
- plistlib: 生成和解析Apple .plist文件

### 加密服务
- hashlib: 安全哈希和消息摘要
- hmac: 消息认证的键控哈希
- secrets: 生成管理机密的安全随机数

### 通用操作系统服务
- os: 各种操作系统接口
- io: 流处理核心工具
- time: 时间访问和转换
- logging: Python日志记录工具及其配置和处理器
- platform: 访问底层平台标识数据
- errno: 标准errno系统符号
- ctypes: Python外部函数库
...
### 图形用户界面
- tkinter: Python的Tcl/Tk接口及相关组件和工具
- IDLE: Python编辑器和外壳
...
- unittest: 单元测试框架及mock对象库
- test: Python回归测试包及其工具

### 调试和性能分析
- bdb: 调试器框架
- faulthandler: 转储Python回溯
- pdb: Python调试器
- cProfile, profile: Python分析器
- timeit: 测量小代码片段的执行时间
- trace: 跟踪Python语句执行
- tracemalloc: 跟踪内存分配
...
### 导入模块
- zipimport: 从Zip归档导入模块
- pkgutil: 包扩展实用程序
- modulefinder: 查找脚本使用的模块
- runpy: 定位和执行Python模块
- importlib: import的实现，包括资源和元数据访问

### Python语言服务
- ast: 抽象语法树
- symtable: 访问编译器符号表
- token: Python解析树使用的常量
- keyword: 测试Python关键字
- tokenize: Python源代码分词器
- tabnanny: 检测模糊缩进
- pyclbr: Python模块浏览器支持
- py_compile: 编译Python源文件
- compileall: 字节编译Python库
- dis: Python字节码反汇编器
- pickletools: pickle开发者工具
