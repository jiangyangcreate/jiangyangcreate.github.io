...
def draw_add(path):
    ...
    mask_img = np.zeros((img_width, img_height, 3), dtype=np.uint8) + 255
...
def draw_subtract(path):
    ...
    if img is None:
        return 
    h, w = img.shape[:2]
    mask_img = np.zeros((h, w, 3), dtype=np.uint8) + 255
    cv2.circle(mask_img, (w//2, h//2), int(min(w, h)*0.45), (0, 0, 0), -1)
...
- `fx`：水平缩放比例
- `fy`：垂直缩放比例
- `interpolation`：插值方式，默认值为cv2.INTER_LINEAR
