...
df = pd.DataFrame(data)
# 写入 JSONL 文件  
# orient='records'表示每条记录（每行）是一个独立的JSON对象（即列表中的字典形式）
# lines=True 表示每行一个JSON对象
df.to_json('data.jsonl', orient='records', lines=True)
# 读取 JSONL 文件  lines=True 表示每行一个JSON对象
...
Pandas 内部集成了 Matplotlib。当你处理表格数据（DataFrame）时，可以直接调用 `.plot()` 方法。它非常适合快速查看数据分布。
