...
:::info
接口是一组定义了不同软件组件之间如何交互的规则和协议。也叫<HoverText text="API" explanation="Application Programming Interface，应用程序编程接口"/>。
...
下面是一个实现屏幕共享的示例，使用`StreamingResponse`实现流式响应：
...
- 用户上传大视频文件（如 1GB），系统需要对视频进行转码（CPU 密集型操作，耗时数分钟）。
- 用户通过提示词生成视频（GPU 密集型操作，耗时数分钟）。

系统需要立即返回"正在处理中"，而非让用户等待转码完成。

- 在 `async def` 里直接写转码逻辑不行，会阻塞整个事件循环
- 在 `def` 里写转码逻辑，靠线程池不行，不能立刻返回"正在处理中"
- 使用 BackgroundTasks（FastAPI 内置后台任务），可以，但是任务仅存在于内存中，如果 FastAPI 服务重启或崩溃，正在生成或排队中的视频任务会直接丢失，无法恢复。
- 使用 Celery + Redis（分布式任务队列），可以，复杂的配置带来完善的功能：自动重试、消息持久化都支持。
...
- **`background_tasks: BackgroundTasks`**：通过依赖注入获取 BackgroundTasks 实例
- **`background_tasks.add_task()`**：添加后台任务，第一个参数是函数，后续参数是函数的参数
- **立即返回响应**：在添加后台任务后立即返回，不等待任务完成
- **任务执行时机**：后台任务会在响应发送给客户端**之后**才开始执行
...
Celery 是 Python 下强大的分布式任务队列，支持多种后端，包括 Redis 、RabbitMQ 等。

Redis-py 是 Redis 的 Python [官方客户端](https://redis.readthedocs.io/en/latest/)，支持同步与异步，提供字符串、哈希、列表、集合、有序集合及连接池、Pipeline、Pub/Sub 等。
...
安装 Redis 数据库推荐通过 Docker 安装 `docker run -d --name redis -p 6379:6379 redis`

安装 Redis-py 和 Celery：
