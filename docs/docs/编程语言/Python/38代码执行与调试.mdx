...
- code: 提供创建自定义交互式解释器（REPL）、嵌入式控制台等的工具，便于在程序内部打开一段“临时 Python 终端”进行调试。
- dis: 反汇编 Python 字节码，帮助你观察某段源码在解释器层面的实际执行指令，对性能分析和深入理解语言实现很有用。
- keyword: 提供 Python 关键字列表及检查接口，可用于高亮、代码生成时避免使用关键字作为变量名等。