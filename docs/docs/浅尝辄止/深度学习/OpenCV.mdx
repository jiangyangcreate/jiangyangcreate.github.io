...
如果img是二维数组（灰度图）根据其数据类型，imshow做不同处理：
- 浮点数的二维数组，imshow的时候，会把0当作黑。1.0及以上当作白，0~1中间的是灰色。
- 整型的二维数组，imshow的时候，会把0当作黑。255及以上当作白，0~255中间的是灰色。
...
img1 = cv2.imread('desktop_avatar.jpg')
print(img1.shape)
...
gray:NDArray = B*0.114 + G*0.587 + R*0.299 
int_gray = gray.astype(np.uint8)
float_gray = gray/255
# 浮点数的二维数组，imshow的时候，会把0当作黑。1.0及以上当作白，0~1中间的是灰色。
# 整型的二维数组，imshow的时候，会把0当作黑。255及以上当作白，0~255中间的是灰色。
cv2.imshow("Image Window1",int_gray)
cv2.imshow("Image Window2",float_gray)
...
font_path = "SIMHEI.TTF"  # 黑体（确保该字体文件存在）
font_size = 60
...
# 计算文本的边界框（使用 textbbox）新版的PIL用textbbox
...
**注意**：
- 对于 `TM_SQDIFF` 和 `TM_SQDIFF_NORMED` 方法，最小值位置是最佳匹配
- 对于其他方法，最大值位置是最佳匹配
...
原始的 YOLOv1~v3 由 Joseph Redmon 用 C+Darknet 实现，他已在2020年退出该领域。

目前最主流、最活跃的 YOLO 系列实现是 **Ultralytics YOLO**（YOLOv11），它完全基于 PyTorch 开发和维护（与原始作者 Joseph Redmon 无关）。

如果说`PyTorch`是强大的深度学习“工具箱”，那么`Ultralytics YOLO`就是用这个工具箱造好的一台“目标检测成品机器”，开箱即用。
