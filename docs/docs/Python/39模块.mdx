...
| `-X pycache_prefix=PATH` | 把 `.pyc` 写到指定目录树，不污染源码目录（等价 `PYTHONPYCACHEPREFIX`）。 |
...
| `-m <module>` | 在 `sys.path` 中查找模块并以 `__main__` 运行（包则运行 `.__main__`）。 | `python -m http.server` |
...
在 **Docker、systemd、K8s** 等环境里，`python -u` 很常见；排查「为什么本地能跑、服务器不行」时，`python -I` 或临时 `python -E` 有助于确认是否被环境变量或路径劫持。
:::
