...
`__slots__`空间占用更小的代价是失去了动态添加属性和弱引用的能力。下面的例子中，你不能添加 `x` 和 `y` 之外的属性。
...
底层的 C 函数（比如 os.stat）和大量的库过去只认 `str` 或 `bytes`。现在，它们会寻找该对象是否有 `__fspath__()` 方法，如果有就用`os.fspath()`来获取路径字符串。
...
:::tip
Python类默认的数据模型是`__dict__`。不论是属性，还是方法，都是存储在`__dict__`中。

getattr的本质就是从`__dict__`中获取对应的值，如果是属性就是获取属性值，如果是方法就是获取方法对象。
:::
