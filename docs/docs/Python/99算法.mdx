...
下面用 numpy 实现 K-means：随机选 K 个质心，反复执行“按最近质心划分—重算质心”直到收敛。

```python showLineNumbers
import numpy as np

def euclidean_distance_points(X, centroid):
    """计算所有样本点 X 到质心 centroid 的距离，返回一维数组。"""
...
def kmeans(X, k, max_iters=100, tol=1e-4):
    ...
    :param tol: 质心变化小于此阈值则收敛
    :return: 每个样本的簇标签, 质心数组
    """
    n_samples = X.shape[0]
    # 1. 随机选 k 个样本作为初始质心
    ...
    for _ in range(max_iters):
        ...
        for j in range(k):
            dists[:, j] = euclidean_distance_points(X, centroids[j])
        ...
        for j in range(k):
            ...
            if np.sum(mask) > 0:
                ...
                new_centroids[j] = centroids[j]  # 空簇保留原质心
...
if __name__ == "__main__":
    # 二维数据，期望聚成 2 类
    X = np.array([[1, 1], [1.5, 2], [3, 3], [5, 5], [6, 6], [7, 7]], dtype=float)
...
以下用 numpy 实现 DBSCAN：先算每点 ε-邻域，标出核心点，再从核心点扩展簇（密度可达），未归属的点为噪声（-1）。
...
def dbscan(X, eps, min_samples):
    """
    DBSCAN 聚类：基于密度的聚类，可形成任意形状的簇并标记噪声点。
    :param X: 样本矩阵 (n_samples, n_features)
    :param eps: ε-邻域半径
    :param min_samples: 核心点的 ε-邻域内最少点数（含自身）
    :return: 每个样本的簇标签，-1 表示噪声
    """
    n = X.shape[0]
    labels = np.full(n, -1)  # -1 表示未分类/噪声
    cluster_id = 0
    ...
    def get_neighbors(i):
        """返回点 i 的 ε-邻域内的点索引列表。"""
        return [j for j in range(n) if distance(X[i], X[j]) <= eps]

    for i in range(n):
        if labels[i] != -1:
            continue
        neighbors = get_neighbors(i)
        # 邻域内点数 < min_samples，标记为噪声
...
def euclidean_distance(a, b):
    """计算欧氏距离，对应章节中的 |AB| = sqrt((x1-x2)^2 + (y1-y2)^2 + ...)"""
    return np.sqrt(np.sum((np.array(a) - np.array(b)) ** 2))

def knn_predict(X_train, y_train, x_new, k=3):
    """
    KNN 分类：找距离未知样本最近的 k 个已知样本，按多数类别预测。
    :param X_train: 已知样本特征，形状 (n_samples, n_features)
    :param y_train: 已知样本类别标签
    :param x_new: 待预测的一个样本特征
    :param k: 近邻个数
    :return: 预测类别
    """
    # 计算 x_new 与所有训练样本的距离
    distances = [euclidean_distance(x_new, x) for x in X_train]
    # 取距离最小的 k 个样本的索引（argsort 升序，取前 k 个）
...
以下实现与上文案例一致：二分类（垃圾/正常），特征为“免费”“恭喜”“辛苦”是否出现；使用拉普拉斯平滑避免概率为 0。

```python showLineNumbers
import numpy as np

class NaiveBayesEmail:
    """朴素贝叶斯邮件二分类：P(类别|特征) 正比于 P(特征|类别) * P(类别)，特征相互独立。"""

    def __init__(self, alpha=1.0):
        """
        :param alpha: 拉普拉斯平滑系数，在每类每个特征计数上加上 alpha，避免 P=0
        """
        self.alpha = alpha
        self.classes = None       # 类别列表，如 [0, 1]
        self.prior = None         # 先验 P(类别)，字典
        self.likelihood = None    # 似然 P(特征|类别)，嵌套字典 likelihood[类别][特征索引]

    def fit(self, X, y):
        """
        X: 二维数组，每行一封邮件，每列表示一个词是否出现 (0/1 或出现次数)
        y: 每封邮件的类别，如 0=正常 1=垃圾
        """
        X = np.asarray(X)
        y = np.asarray(y)
        ...
        for c in self.classes:
            self.prior[c] = (np.sum(y == c) + self.alpha) / (len(y) + n_classes * self.alpha)

        # 似然：P(特征j|类别c)，拉普拉斯平滑 (count + alpha) / (该类总数 + 2*alpha) 二值特征
        self.likelihood = {}
        for c in self.classes:
            mask = y == c
            count_c = np.sum(mask)  # 该类邮件数
            # 该类下每个特征的总出现次数（按列求和）
            feature_counts = np.sum(X[mask], axis=0) + self.alpha
            denom = count_c + 2 * self.alpha  # 二值：0 和 1 各加 alpha
            self.likelihood[c] = feature_counts / denom

    def predict_proba_one(self, x):
        """对单条样本计算 P(类别|特征)，返回各类别概率（未归一化可用于比较）。"""
        probs = {}
        for c in self.classes:
            # P(c) * ∏ P(x_j | c)，取 log 再 exp 避免下溢，这里样本维度小直接乘
            p = self.prior[c]
            for j in range(len(x)):
                if x[j] > 0:
                    p *= self.likelihood[c][j]
                else:
                    p *= (1 - self.likelihood[c][j])
            probs[c] = p
        return probs
    ...
    def predict(self, X):
        """预测每条样本的类别：取概率更大的那一类。"""
        X = np.asarray(X)
        return np.array([max(self.predict_proba_one(x).items(), key=lambda t: t[1])[0] for x in X])

if __name__ == "__main__":
    # 与章节一致：100 封中 80 垃圾 20 正常；特征顺序：免费、恭喜、辛苦
    # 每行一封邮件，1 表示该词出现
    X = np.zeros((100, 3))
    y = np.zeros(100, dtype=int)
    y[:80] = 1  # 前 80 封为垃圾邮件
    # 垃圾邮件：20 封含“免费”，50 封含“恭喜”，0 封含“辛苦”
    X[:20, 0] = 1
    X[:50, 1] = 1
    # 正常邮件：5 封含“免费”，6 封含“恭喜”，2 封含“辛苦”
    X[80:85, 0] = 1
    X[80:86, 1] = 1
    X[98:100, 2] = 1

    model = NaiveBayesEmail(alpha=1.0)
    model.fit(X, y)

    # “恭喜您获得了一次免费的机会” -> 免费、恭喜出现，辛苦未出现
    x_new = np.array([1, 1, 0])
    proba = model.predict_proba_one(x_new)
    print("P(垃圾|免费,恭喜):", proba.get(1), " P(正常|免费,恭喜):", proba.get(0))
    print("预测类别:", "垃圾" if model.predict(x_new.reshape(1, -1))[0] == 1 else "正常")
```
...
下面实现一个基于信息增益的特征选择与简单决策树生成（仅用 numpy，不含剪枝）。信息增益 = 经验熵 H(D) - 按特征 A 划分后的条件熵 H(D|A)，选增益最大的特征作为当前结点划分依据。

```python showLineNumbers
import numpy as np

def entropy(y):
    """经验熵 H(D)：对类别标签 y 计算 -Σ p_k log2(p_k)。"""
    n = len(y)
    if n == 0:
        return 0.0
...
def info_gain(X, y, feature_idx):
    """
    按特征 feature_idx 划分后的信息增益 = H(D) - H(D|A)。
    H(D|A) = Σ (|Dv|/|D|) * H(Dv)，Dv 为特征 A 取值为 v 的样本子集。
    """
    n = len(y)
    H_D = entropy(y)
    col = X[:, feature_idx]
    ...
    for v in values:
        mask = col == v
        Dv = y[mask]
        H_DA += (len(Dv) / n) * entropy(Dv)
    return H_D - H_DA

def fit_tree(X, y, feature_names=None, depth=0, max_depth=10):
    """
    递归生成决策树（字典表示）。叶结点存类别；非叶结点存特征索引、划分值（或类别值）与子结点。
    这里简化为离散特征：按特征取值分支。
    """
    if feature_names is None:
        feature_names = [str(i) for i in range(X.shape[1])]
    # 叶结点：样本同属一类或达到最大深度，取多数类
    if len(np.unique(y)) <= 1 or depth >= max_depth:
        return {"leaf": int(np.bincount(y.astype(int)).argmax())}
    ...
    for j in range(X.shape[1]):
        ...
        if g > best_gain:
            best_gain = g
            best_idx = j
    if best_idx < 0 or best_gain <= 1e-9:
        return {"leaf": int(np.bincount(y.astype(int)).argmax())}

    node = {"feature": best_idx, "name": feature_names[best_idx], "children": {}}
    col = X[:, best_idx]
    for v in np.unique(col):
        mask = col == v
        node["children"][v] = fit_tree(X[mask], y[mask], feature_names, depth + 1, max_depth)
    return node
...
def predict_one(tree, x):
    ...
    if v not in tree["children"]:
        return -1  # 未见过的取值，可返回默认类
    return predict_one(tree["children"][v], x)

if __name__ == "__main__":
    # 简单示例：2 个特征，2 类
    X = np.array([[0, 0], [0, 1], [1, 0], [1, 1], [1, 0], [0, 1]])
    y = np.array([0, 0, 1, 1, 1, 0])
    tree = fit_tree(X, y, feature_names=["A", "B"], max_depth=5)
    print("决策树结构（字典）:", tree)
    for i in range(len(X)):
        print("样本", X[i], "预测:", predict_one(tree, X[i]))
```
...
class Line:
    ...
    def train(self, epoch_times):
        """训练：每轮对每个样本求梯度并更新 w、b，损失为 (y - y_pred)^2。"""
        for epoch in range(epoch_times):
            total_loss = 0
            for x, y in zip(self.X, self.y):
                y_pred = self.predict(x)
                # 损失 loss = (y - (wx + b))^2，对 w 求偏导得 -2*x*(y - y_pred)，对 b 求偏导得 -2*(y - y_pred)
...
class Sline:
    """逻辑回归：用 sigmoid(w*x + b) 拟合二分类概率，均方误差损失，梯度下降更新 w、b。"""
    ...
    def sigmoid(self, x):
        """激活函数 f(z) = 1/(1+e^{-z})，z = w*x + b。"""
        return 1 / (1 + np.exp(-(self.w * x + self.b)))

    def train(self, epoch_times):
        """训练：loss = (y - y_pred)^2，y_pred = sigmoid(w*x+b)；对 w、b 求梯度并更新。"""
        for epoch in range(epoch_times):
            total_loss = 0
            for x, y in zip(self.X, self.y):
