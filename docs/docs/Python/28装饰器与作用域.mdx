...
:::tip
<Highlight>Python在编译函数时会静态分析作用域，而非运行时动态判断</Highlight>
...
- 一旦函数体内出现 x = 5（对 x 的赋值），Python立即将 x 标记为局部变量
- 因此 print(x) 会在局部作用域查找 x，但此时 x 尚未赋值（运行时），导致 UnboundLocalError
- 关键点：不是"局部不能修改全局"，而是"局部变量在赋值前被引用"
:::
...
`locals()` 和 `globals()` 都是用来**查看当前命名空间中的变量**的内置函数，本质上都是**返回一个字典对象**。
...
> 一般来说：
>
> - 在**函数内部**：`locals()` 反映的是当前函数的局部变量，`globals()` 反映的是整个模块的全局变量；
> - 在**模块顶层**（不在函数里）：`locals()` 和 `globals()` 返回的其实是同一个字典（都是全局命名空间）。
