...
函数签名：`open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None) -> file objec`
...
内存中文本流也可以作为 StringIO 对象使用：`f = io.StringIO("some initial text data")`
...
迭代文件对象，逐行读取内容。从文件中读取多行时，可以用循环遍历整个文件对象。这种操作能高效利用内存，快速，且代码简单：

```python showLineNumbers
f = open('test.txt')
# 即便 test.txt 是一个 100GB 的巨型文件，这段代码依然能平稳运行。
for line in f:
    print(line, end='') # end='' 防止额外换行
f.close()
```

如需以列表形式读取文件中的所有行，可以用 `list(f)` 或 `f.readlines()`。但是可能会内存溢出。

如果你的文件非常特殊（比如一个没有换行符的巨型 JSON 文件），那么 for line in f 依然会因为找不到换行符而尝试读取超大块数据，导致内存告急。

在这种极端情况下，你需要按字节块（Chunk）读取：

```python showLineNumbers
with open('test.txt', 'rb') as f:
    while True:
        chunk = f.read(4096) # 每次只读 4KB
        if not chunk:
            break
        process(chunk)
```
...
内存中二进制流也可以作为 BytesIO 对象使用：`f = io.BytesIO(b"some initial binary data: \x00\x01")`
