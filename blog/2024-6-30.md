...
tags: [smarthome]
keywords: [smarthome]
description: 记录个人实现通过RS323协议手册进行USB控制
---

购买了了一款摄像头，默认能通过 RS323 协议控制，但是现在新的主板已经没有这种圆形的接口了，基本都是 USB，扔之可惜，尝试自己编写逻辑代码控制。咨询商家后，商家提供了 RS232 协议的指令集，所以这里通过 python 的 serial 库尝试通过 USB 口对其云台调用。

编写好控制指令之后，就可以通过键盘来控制摄像头的旋转了，监听键盘事件，调用指定运动到合适角度之后停止。

调整好之后可以按下空格暂停，按下 ESC 退出控制程序。
...
这里我们观察范例，对数据做了拆分，即如果需要把10进制映射到16进制上，譬如17转成16进制是11，那么应该变成0101。每位之间插入`0`。
...
# 前方补0到总长度为8位，结果和我们预期一致
print(calculate_pan_position_bytes(17))
# 00000101
```

接下来通过代入0到6000这样数值传输给串口发现，只能向左转。

0对应居中，4500对应向左转180，数字再大也是转到底。

4500比180 = 25比1，所以我们输入角度，乘以25就得到了对应的信号值。

按手册说明水平转动范围为355度，一半则是177.5度，与肉眼观察基本一致，Z轴的范围是上下各21度。

刚刚只能向左转，那么向右转的答案就呼之欲出了，要么是补码（异或运算后加1），要么首位是符号位。我们添加上限位和映射，先用补码试试完成这个函数（结果直接成了）。
