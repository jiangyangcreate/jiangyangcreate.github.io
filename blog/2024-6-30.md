...
购买了一款摄像头，默认能通过 RS232 协议控制，但是现在新的主板已经没有这种圆形的接口了，基本都是 USB，扔之可惜，尝试自己编写逻辑代码控制。咨询商家后，商家提供了 RS232 协议的指令集，所以这里通过 python 的 serial 库尝试通过 USB 口对其云台调用。

编写好控制指令之后，就可以通过键盘来控制摄像头的旋转了，监听键盘事件，调用云台运动的指令，运动到合适角度之后可以按下空格停止云台运动，按下 ESC 退出控制程序。
